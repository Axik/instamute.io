/**
 * @license  OpenTok JavaScript Library v2.0.19.2
 * http://www.tokbox.com/
 *
 * Copyright (c) 2013 TokBox, Inc.
 *
 * Date: May 01 08:14:52 2014
 */
...
(function(window) {
    ...
    OT.properties = {
        version: "v2.0.19.2", // The current version (eg. v2.0.4) (This is replaced by gradle)
        build: "ad7908c", // The current build hash (This is replaced by gradle)
        ...
        apiURL: "https://anvil.opentok.com", // The anvil API URL
...
/**
 * @license  Common JS Helpers on OpenTok 0.1.0 dc33460 master-webrtc-2.0
 * http://www.tokbox.com/
 *
 * Copyright (c) 2014 TokBox, Inc.
 *
 * Date: March 17 05:28:21 2014
 *
 */
...
(function(window) {
    ...
    var getOrCreateContainer = function getOrCreateContainer(elementOrDomId, insertMode) {
        ...
        if (!container) {
            container = OT.$.createElement('div', {
                id: domId
            });
...
(function(window) {
    ...
    var nativeGetUserMedia = (function() {
    ...
    //
    OT.$.getUserMedia = function(constraints, success, failure, accessDialogOpened, accessDialogClosed, accessDenied, customGetUserMedia) {
...
(function(global) {
    ...
    OT.Rumor.Message.Connect = function(uniqueId, notifyDisconnectAddress) {
        var headers = [{
            ...
            key: 'notifyDisconnectAddress',
            val: notifyDisconnectAddress
        }];
    ...
    OT.Raptor.Socket = function(widgetId, messagingServer, Dispatcher) {
        ...
        var setState = OT.$.statable(this, ['disconnected', 'connecting', 'connected', 'error', 'disconnecting'], 'disconnected'),
            ...
            onConnectComplete = function onConnectComplete(error, sessionState, prefix) {
                if (error) {
                    ...
                    logAnalyticsEvent('Success', "webSocketServerUrl", _socketUrl, {
        ...
        this.connect = function(token, sessionInfo, completion) {
            ...
            _rumor.connect(connectionId, function(error) {
                if (error) {
                    onConnectComplete(error, null, "WebSocketConnection:");
                    return;
                }
...
(function(global) {
    ...
    OT.Raptor.Dispatcher.prototype.dispatchJsep = function(message) {
        ...
        switch (message.action) {
            ...
            case OT.Raptor.Actions.OFFER:
                actors = [];
                var subscriber = OT.subscribers.find({
                    streamId: streamId
                });
    ...
    OT.Raptor.Dispatcher.prototype.dispatchSubscriber = function(message) {
        ...
        subscriber = OT.subscribers.find(function(subscriber) {
            return subscriber.streamId === stream.id && subscriber.session.id === session.id
        });
...
(function(window) {
    ...
    function _exceptionHandler(component, msg, errorCode, context) {
        ...
        try {
            ...
            analytics.logError(errorCode, 'tb.exception', title, {
                details: msg
            }, contextCopy);
...
(function(window) {
    ...
    /**
 * The Connection object represents a connection to an OpenTok session. Each client that connects to
 * a session has a unique connection, with a unique connection ID (represented by the
 ...
 * <p>
 * The Session object dispatches a <code>sessionConnected</code> event when you connect to a
 * session. The <code>sessionConnected</code> event object includes a <code>connections</code>
 * property, which is an array of Connection objects corresponding to the clients (other than your
 * own) that are present in the session when you connect. The Session object dispatches a
 * <code>connectionCreated</code> other clients connect to a session. The
 * <code>connectionCreated</code> event object includes a <code>connections</code> property, which
 * is an array of Connection objects corresponding to the clients the event pertains to.
 * <p>
 * The Stream object has a <code>connection</code> property that is a Connection object.
 * It represents the connection of the client that's publishing the stream.
 *
 * @class Connection
...
(function(window) {
    ...
    OT.Stream = function(id, connection, name, data, type, creationTime, hasAudio, hasVideo, orientation, peerId, quality, width, height) {
        ...
        this.videoDimensions = {
            width: (width || 640),
            height: (height || 480),
            orientation: (orientation || OT.VideoOrientation.ROTATED_NORMAL)
        };
                    ...
                    if (_peerConnection.oniceconnectionstatechange !== undefined) {
                        ...
                        _peerConnection.oniceconnectionstatechange = function(event) {
                            if (event.target.iceConnectionState === 'failed') {
                                ...
                                setTimeout(function() {
                                    if (event.target.iceConnectionState === 'failed') {
                                        triggerError('ICEWorkflow:The stream was unable to connect due to a network error.' +
...
(function(window) {
    ...
    OT.SessionInfo.get = function(session, onSuccess, onFailure) {
        var sessionInfoURL = OT.properties.apiURL + '/session/' + session.id + "?extended=true",
            ...
            validateRawSessionInfo = function(sessionInfo, responseText) {
                session.logEvent('Instrumentation', null, 'gsi', OT.$.now() - startTime);

                var error = parseErrorFromXMLDocument(sessionInfo, responseText);
        ...
        OT.$.getXML(sessionInfoURL, {
            ...
            success: function(sessionInfo, event) {
                validateRawSessionInfo(sessionInfo, event.target.responseText)
            },

            error: function(event) {
                onGetErrorCallback(session, onFailure, parseErrorFromXMLDocument(event.target.responseXML, event.target.responseText));
            }
        });
    };
    ...
    // false.
    parseErrorFromXMLDocument = function(xmlDocument, responseText) {
        if (xmlDocument && xmlDocument.documentElement && xmlDocument.documentElement.firstElementChild !== null) {
    ...
    onGetErrorCallback = function(session, onFailure, error) {
        ...
        session.logEvent('Connect', 'Failure', 'errorMessage', "GetSessionInfo:" + messageServerToClientErrorCodes[error.code] + ": Unable to get session info " + error.message);
        ...
        return iceServers.map(function(iceServer) {
            ...
            if (userAgent !== null) {
                ...
                if (firefoxVersion < 25) {
                    return {
                        url: iceServer.url.replace("turn:", "stun:")
                    };
                }
...
(function(window) {
    ...
    OT.Session = function(sessionId) {
        ...
        sessionConnectFailed = function(reason, code) {
            ...
            connectionCreatedHandler = function(connection) {
                ...
                this.dispatchEvent(new OT.ConnectionEvent(
                    OT.Event.names.CONNECTION_CREATED, [connection]
                ));
            },
            ...
            connectionDestroyedHandler = function(connection, reason) {
                ...
                this.dispatchEvent(
                    new OT.ConnectionEvent(
                        OT.Event.names.CONNECTION_DESTROYED, [connection],
                        reason
                    )
                );
            },
            ...
            streamCreatedHandler = function(stream) {
                this.dispatchEvent(new OT.StreamEvent(
                    OT.Event.names.STREAM_CREATED, [stream]
                ));
            },
            ...
            streamDestroyedHandler = function(stream, reason) {
                ...
                var defaultAction = function() {
                    if (!event.isDefaultPrevented()) {
                        ...
                        // a publisher.
                        var publisher = OT.publishers.where({
                            streamId: stream.id
                        })[0];
                        if (publisher) {
        ...
        OT.$.defineGetters(this, {
            ...
            capabilities: function() {
                return _socket ? _socket.capabilities : new OT.Capabilities([]);
            },
            sessionId: function() {
                return _sessionId;
            },
